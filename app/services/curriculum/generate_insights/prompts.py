...
def build_taxonomy_prompt(
    ...
    - topic 이름은 소문자 영문 snake_case 로 작성한다.
    예: "python_basic", "python_advanced", "pandas", "nlp", "career", "portfolio", "ide_setting"
...
def build_classification_prompt(
    ...
    - pattern_tags: 아래 중에서 상황에 맞게 복수 선택
        - "concept_confusion"      # 개념 헷갈림, 정의/원리 이해 부족
        - "api_usage"              # 라이브러리/함수/메서드 사용법 질문
        - "expected_output_mismatch" # 내가 예상한 결과와 실제 결과가 다를 때
        - "environment_issue"      # IDE, 패키지 설치, 버전 문제, 경로 문제 등 환경 이슈
        - "other"                  # 위에 딱 맞지 않으면
    - intent:
